"""Riverside County Assessor scraper using Playwright browser automation."""
...
# ---------------------------------------------------------------------------
# Riverside County assessor portal configuration
# ---------------------------------------------------------------------------
# NOTE: These URLs may need adjustment as the county website changes.
RIVERSIDE_ASSESSOR_BASE_URL = "https://rivcoassessor.org"
RIVERSIDE_ASSESSOR_SEARCH_URL = f"{RIVERSIDE_ASSESSOR_BASE_URL}/property-search"

# Timeout for navigation and element waits (ms)
DEFAULT_TIMEOUT_MS = 30_000


class RiversideCountyScraper(BaseScraper):
    """Scrapes commercial property data from the Riverside County assessor portal.

    Uses Playwright to automate a headless Chromium browser that navigates
    the assessor property search interface, filters for commercial properties,
    and parses result tables.
    """

    def __init__(self, county_name: str = "Riverside") -> None:
    ...
    async def _navigate_to_search(self) -> None:
        """Navigate to the Riverside County assessor property search page."""
        assert self._page is not None
        await self._page.goto(RIVERSIDE_ASSESSOR_SEARCH_URL, wait_until="networkidle")
    ...
    async def _perform_search(self, page_num: int) -> None:
        ...
        Searches for commercial zoned properties on the Riverside County
        assessor portal.  The actual selectors will need to be tuned to
        match the live site's DOM structure.
        """
        assert self._page is not None
        ...
        if page_num == 1:
            ...
            try:
                use_type_selector = self._page.locator(
                    "select[name='UseType'], select[name='propertyUse'], "
                    "#UseType, #propertyUse, select[name='use_type']"
                )
    ...
    async def _go_to_page(self, page_num: int) -> None:
        ...
        try:
            next_btn = self._page.locator(
                ...
                "a.next, .pagination .next, .paging .next"
            )
    ...
    async def _parse_results_table(self) -> list[dict]:
        ...
        table = self._page.locator(
            "table.results, table.search-results, table.property-results, table"
        )
    ...
    def _map_row_to_property(self, cells: list[str]) -> dict | None:
        ...
        The column order assumed here is based on the Riverside County
        assessor portal layout.  Adjust indices as needed.
    ...
    async def scrape(self, page_num: int) -> list[dict]:
        """Scrape a single page of property results from Riverside County.
        ...
        """
        self.log.info("scraping_riverside_county_page", page=page_num)
