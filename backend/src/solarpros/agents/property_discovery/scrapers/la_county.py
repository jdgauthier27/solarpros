"""LA County Assessor scraper using Playwright browser automation."""
...
# ---------------------------------------------------------------------------
# LA County assessor portal configuration
# ---------------------------------------------------------------------------
# NOTE: These URLs may need adjustment as the county website changes.
LA_ASSESSOR_BASE_URL = "https://portal.assessor.lacounty.gov"
LA_ASSESSOR_SEARCH_URL = f"{LA_ASSESSOR_BASE_URL}/parceldetail"

# Timeout for navigation and element waits (ms)
DEFAULT_TIMEOUT_MS = 30_000


class LACountyScraper(BaseScraper):
    """Scrapes commercial property data from the LA County assessor portal.
    ...
    def __init__(self, county_name: str = "Los Angeles") -> None:
    ...
    async def _navigate_to_search(self) -> None:
        """Navigate to the assessor search page."""
        assert self._page is not None
        await self._page.goto(LA_ASSESSOR_SEARCH_URL, wait_until="networkidle")
    ...
    async def _perform_search(self, page_num: int) -> None:
        ...
        This performs a search for commercial zoned properties. The
        actual selectors will need to be tuned to match the live site's
        DOM structure.
        """
        assert self._page is not None
        ...
        if page_num == 1:
            ...
            # Try to find and interact with the use-class / property-type filter
            # The exact selector depends on the site's current UI
            try:
                use_type_selector = self._page.locator(
                    "select[name='UseType'], select[name='propertyType'], "
    ...
    async def _parse_results_table(self) -> list[dict]:
        ...
        table = self._page.locator("table.results, table.search-results, table.data-table, table")
        ...
        for i in range(row_count):
            try:
                ...
                # Column mapping will need to be adjusted based on the actual
                # table structure of the LA County assessor portal
    ...
    def _map_row_to_property(self, cells: list[str]) -> dict | None:
        ...
        The column order assumed here is based on typical county
        assessor result tables. Adjust indices as needed.
    ...
    async def scrape(self, page_num: int) -> list[dict]:
        """Scrape a single page of property results from LA County.
        ...
        """
        self.log.info("scraping_la_county_page", page=page_num)

        await self._perform_search(page_num)

        # Small delay to be polite to the county server
        await asyncio.sleep(2)

        properties = await self._parse_results_table()
        self.log.info(
            "la_county_page_scraped",
            page=page_num,
