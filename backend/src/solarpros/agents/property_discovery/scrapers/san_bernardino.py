"""San Bernardino County Assessor scraper using Playwright browser automation."""
...
# ---------------------------------------------------------------------------
# San Bernardino County assessor portal configuration
# ---------------------------------------------------------------------------
# NOTE: These URLs may need adjustment as the county website changes.
SB_ASSESSOR_BASE_URL = "https://www.sbcounty.gov/assessor"
SB_ASSESSOR_SEARCH_URL = f"{SB_ASSESSOR_BASE_URL}/property-search"

# Timeout for navigation and element waits (ms)
DEFAULT_TIMEOUT_MS = 30_000


class SanBernardinoCountyScraper(BaseScraper):
    """Scrapes commercial property data from the San Bernardino County assessor portal.
    ...
    def __init__(self, county_name: str = "San Bernardino") -> None:
    ...
    async def _navigate_to_search(self) -> None:
        """Navigate to the SB County assessor property search page."""
        assert self._page is not None
        await self._page.goto(SB_ASSESSOR_SEARCH_URL, wait_until="networkidle")
    ...
    async def _perform_search(self, page_num: int) -> None:
        ...
        Searches for commercial zoned properties on the SB County
        ...
        if page_num == 1:
            ...
            try:
                use_type_selector = self._page.locator(
                    ...
                    "#UseType, #PropertyUse, select[name='property_type']"
                )
    ...
    async def _go_to_page(self, page_num: int) -> None:
        ...
        try:
            next_btn = self._page.locator(
                ...
                "a.next, .pagination .next, .pager .next"
            )
    ...
    async def _parse_results_table(self) -> list[dict]:
        ...
        table = self._page.locator(
            "table.results, table.search-results, table.data-grid, table"
        )
    ...
    def _map_row_to_property(self, cells: list[str]) -> dict | None:
        ...
        The column order assumed here is based on the SB County assessor
    ...
    async def scrape(self, page_num: int) -> list[dict]:
        """Scrape a single page of property results from San Bernardino County.
        ...
        """
        self.log.info("scraping_san_bernardino_county_page", page=page_num)
        ...
        self.log.info(
            "san_bernardino_county_page_scraped",
            page=page_num,
