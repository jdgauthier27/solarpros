...
resource "aws_secretsmanager_secret" "app_env" {
  ...
  description             = "Environment variables for the ${var.project_name} application"
...
resource "aws_secretsmanager_secret_version" "app_env" {
  ...
  secret_string = jsonencode({
    DATABASE_URL = "postgresql://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.endpoint}/${var.project_name}"
...
resource "aws_ecs_service" "worker" {
  for_each = toset(local.worker_queues)
  ...
  tags = { Name = "${local.name_prefix}-worker-${each.key}" }
}
