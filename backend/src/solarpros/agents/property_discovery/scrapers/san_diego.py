"""San Diego County Assessor scraper using Playwright browser automation."""
...
# ---------------------------------------------------------------------------
# San Diego County assessor portal configuration
# ---------------------------------------------------------------------------
# NOTE: These URLs may need adjustment as the county website changes.
SD_ASSESSOR_BASE_URL = "https://arcc-detail.sdcounty.ca.gov"
SD_ASSESSOR_SEARCH_URL = f"{SD_ASSESSOR_BASE_URL}/propdetail"
...
class SanDiegoCountyScraper(BaseScraper):
    """Scrapes commercial property data from the San Diego County assessor portal.

    Uses Playwright to automate a headless Chromium browser that navigates
    the ARCC property detail interface, filters for commercial properties,
    and parses result tables.
    """

    def __init__(self, county_name: str = "San Diego") -> None:
    ...
    async def _navigate_to_search(self) -> None:
        """Navigate to the SD County assessor property search page."""
        assert self._page is not None
        await self._page.goto(SD_ASSESSOR_SEARCH_URL, wait_until="networkidle")
    ...
    async def _perform_search(self, page_num: int) -> None:
        ...
        Searches for commercial zoned properties on the SD County ARCC
        ...
        if page_num == 1:
            ...
            try:
                use_type_selector = self._page.locator(
                    "select[name='UseType'], select[name='propType'], "
                    "#UseType, #propType, select[name='use_code']"
                )
    ...
    async def _parse_results_table(self) -> list[dict]:
        ...
        # Try to locate the results table (ARCC uses various table classes)
        table = self._page.locator(
            "table.results, table.search-results, table.gridview, table"
        )
    ...
    def _map_row_to_property(self, cells: list[str]) -> dict | None:
        ...
        The column order assumed here is based on the SD County ARCC
        detail portal layout.  Adjust indices as needed.
    ...
    async def scrape(self, page_num: int) -> list[dict]:
        """Scrape a single page of property results from San Diego County.
        ...
        """
        self.log.info("scraping_san_diego_county_page", page=page_num)
